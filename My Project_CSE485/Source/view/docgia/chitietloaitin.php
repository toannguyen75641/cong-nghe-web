      ...
      <div class="card">
        <?php if(isset($list)) { ?>
          <?php foreach ($list as $item) { ?>
            <h2><a href="http://localhost:8888/BTL/trangchu.php?action=xem&id=<?php echo $item['id']; ?>" style="text-decoration: none; color: black;"><?php echo $item['tieu_de']; ?></a></h2>
            <!-- <h5><?php echo date('d/m/Y - H:i:s'); ?></h5> -->
            <div class="fakeimg" style="height: 387px; width: 52%; background-color: white">
              <a href="http://localhost:8888/BTL/trangchu.php?action=xem&id=<?php echo $item['id']; ?>"><img src=<?php echo $item['hinh_anh'] ?>></a>
            </div>
              <p>(Flash News)</p>
              <p><?php echo $item['tom_tat']; ?></p></br>
              <div class="clear"></div>
          <?php } ?>
        <?php } ?>
        </div>
